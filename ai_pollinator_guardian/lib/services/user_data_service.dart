...
class UserDataService {
  ...
  required List<String> photoUrls,
    ...
    try {
      ...
      // Upload garden images to Firebase Storage
      final List<String> photoUrls = [];
      for (int i = 0; i < gardenImages.length; i++) {
        final File image = gardenImages[i];
        final String fileName = 'garden_image_$i.jpg';
        final String folder = 'users/$uid/gardens';

        try {
          final String photoUrl = await _storageService.uploadFile(
            file: image,
            folder: folder,
            fileName: fileName,
          );
          photoUrls.add(photoUrl);
          debugPrint('Uploaded image $i: $photoUrl');
        } catch (e) {
          debugPrint('Error uploading image $i: $e');
          return false; // Stop saving if any image upload fails
        }
      }
  ...
  Future<List<Map<String, dynamic>>> getGardenProfilesByUser() async {
    try {
      ...
      final List<Map<String, dynamic>> gardenProfiles =
          gardens.map((garden) {
            ...
            return {
              ...
              'recommended_plants':
                  gardenData['response']['recommended_plants'],
