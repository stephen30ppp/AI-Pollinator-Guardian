...
class StorageService {
  ...
  Future<void> createFolderIfAbsent(String folder) async {
    ...
    try {
      ...
      throw StorageException('Unexpected error creating folder', originalError: e);
    }
  }
  // endregion
  ...
  Future<String> uploadFile({
    ...
    try {
      ...
      debugPrint('✅ File uploaded successfully: $name (${snapshot.bytesTransferred} bytes)');
      ...
      throw StorageException('Unexpected error uploading file', originalError: e);
    }
  }
  ...
  Future<String> uploadBytes({
    ...
    debugPrint('📤 Uploading bytes: $fileName (${bytes.length} bytes) to folder: $folder');
    ...
    try {
      ...
      // Wait for completion
      final snapshot = await task;
      debugPrint('✅ Bytes uploaded successfully: $fileName (${snapshot.bytesTransferred} bytes)');
      ...
      throw StorageException('Unexpected error uploading bytes', originalError: e);
    }
  }
  ...
  Future<List<String>> uploadFiles({
    ...
    for (final (idx, file) in files.indexed) {
      try {
        debugPrint('📄 Uploading file ${idx + 1}/${files.length}: ${file.path}');
        ...
        debugPrint('📊 Batch progress: ${(progress * 100).toStringAsFixed(1)}%');
      } catch (e) {
    ...
    if (failedCount > 0) {
      debugPrint('⚠️ Completed batch upload with $failedCount failed files. Successfully uploaded: ${urls.length}/${files.length}');
    } else {
  ...
  Future<T> _retryUpload<T>(
    ...
    while (true) {
      ...
      try {
        ...
        final bool isRetryable = e.code == 'network-request-failed' || 
                                e.code == 'unavailable' ||
        ...
        debugPrint('⚠️ Retryable error (${e.code}), attempt $attempts/$maxAttempts. Retrying in ${delay.inMilliseconds}ms...');
  ...
  Future<List<Reference>> listAllFiles(String folder) async {
    ...
    try {
      ...
      debugPrint('✅ Found ${result.items.length} files and ${result.prefixes.length} subfolders in $folder');
      return result.items;
    } on FirebaseException catch (e) {
      ...
      throw StorageException('Unexpected error listing files', originalError: e);
    }
  }
  ...
  Future<ListResult> listFilesPaged(
    ...
    debugPrint('📋 Listing files in folder (paged): $folder (max: $maxResults, token: ${nextPageToken?.substring(0, min(10, nextPageToken.length)) ?? "null"})');
    
    try {
      final result = await _storage.ref(folder).list(
        ListOptions(
      ...
      debugPrint('✅ Found ${result.items.length} files. Has more: ${result.nextPageToken != null}');
      return result;
    } on FirebaseException catch (e) {
      debugPrint('❌ Firebase error listing files (paged): ${e.code} - ${e.message}');
      throw StorageException(
        'Failed to list files: ${e.message}',
        code: e.code,
        originalError: e,
  ...
  Future<String> getDownloadUrl(Reference ref) async {
    ...
    try {
      ...
      debugPrint('❌ Firebase error getting download URL: ${e.code} - ${e.message}');
      throw StorageException(
      ...
      throw StorageException('Unexpected error getting download URL', originalError: e);
    }
  }
  ...
  Future<FullMetadata> getMetadata(Reference ref) async {
    ...
    try {
      ...
      debugPrint('✅ Got metadata. Content type: ${metadata.contentType}, size: ${metadata.size} bytes');
      return metadata;
    } on FirebaseException catch (e) {
      ...
      throw StorageException('Unexpected error getting metadata', originalError: e);
    }
  }
  // endregion
  ...
  Future<void> deleteFileOrFolder(Reference ref) async {
    ...
    try {
      ...
      if (e.code == 'object-not-found') {
        debugPrint('⚠️ File or folder not found (already deleted): ${ref.fullPath}');
      } else {
        debugPrint('❌ Firebase error deleting file/folder: ${e.code} - ${e.message}');
        throw StorageException(
      ...
      throw StorageException('Unexpected error deleting file/folder', originalError: e);
    }
  }
    ...
    try {
      ...
      for (final item in items) {
        try {
          await item.delete();
          deletedCount++;
          debugPrint('✅ Deleted file ${deletedCount}/${items.length}: ${item.fullPath}');
        } catch (e) {
      ...
      debugPrint('✅ Deleted $deletedCount/${items.length} files in folder: $folder');
    } on FirebaseException catch (e) {
      debugPrint('❌ Firebase error deleting folder recursively: ${e.code} - ${e.message}');
      throw StorageException(
      ...
      throw StorageException('Unexpected error deleting folder', originalError: e);
    }
  }
  // endregion
